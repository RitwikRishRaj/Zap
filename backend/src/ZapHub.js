 ...
 *
 * This single Durable Object instance replaces all of:
 *   - backend/src/websocket/roomManager.js
 *   - backend/src/websocket/signaling.js
 *   - backend/src/websocket/relay.js
 *   - backend/src/services/rateLimiter.js
 *
 * One global "hub" instance (keyed "global") handles ALL websocket
 * connections and room state. The WebSocket Hibernation API is used
...
export class ZapHub {
  constructor(state, env) {
    ...
    /**
     * In-memory state — survives normal WebSocket hibernation
     * (DO instance stays alive as long as WS connections exist).
     * Rebuilt from empty if the DO is truly evicted (rare edge case).
     *
     * rooms   : Map<roomId,   RoomObject>
     * ipCount : Map<ip,       number>      — room-creation rate limiting per IP
     * relays  : Map<senderId, RelayObject>
     */
    this.rooms   = new Map();
  ...
  // DO HTTP Interface
  // Called by the main Worker (index.js) for internal queries.
  ...
  async #handleWsUpgrade(request) {
    ...
    // without dropping the connection. ClientId is stored as an attachment
    // so it survives the DO waking back up.
    this.state.acceptWebSocket(server);

    const clientId  = generateClientId();
    const clientIp  = request.headers.get('CF-Connecting-IP') || '0.0.0.0';

    // Attachment: small JSON serialized onto the WS object, survives hibernation
    server.serializeAttachment({ clientId, clientIp, roomId: null });
  ...
  /** Called by CF runtime when a hibernated WS receives a message. */
  async webSocketMessage(ws, message) {
    const attachment = ws.deserializeAttachment() || {};
    const { clientId, clientIp } = attachment;
    ...
    await this.#route(ws, clientId, clientIp, parsed, attachment);
  }

  /** Called when a WS closes (normal or error). */
  async webSocketClose(ws, code, reason, wasClean) {
    const { clientId, roomId } = ws.deserializeAttachment() || {};
    if (clientId) this.#handleDisconnection(clientId, roomId);
  }
  ...
  #createRoom(ws, clientId, clientIp, data = {}, attachment) {
    ...
    const room = {
      ...
      createdAt:   Date.now(),
      expiresAt,
      // clients: Map<clientId, { ws, metadata, joinedAt }>
    ...
    // Persist roomId in WS attachment
    ...
    // Broadcast creator joining to any existing members (edge case, usually empty)
    this.#broadcastToRoom(roomId, clientId, {
  ...
  #joinRoom(ws, clientId, data = {}, attachment) {
    ...
    this.#send(ws, {
      type: 'room_joined',
      roomId,
      room: { id: roomId, mode: room.mode, expiresAt: room.expiresAt, clientCount: room.clients.size },
    });
  ...
  #forwardSignal(clientId, roomId, data = {}) {
    ...
    // Broadcast '*' targets all peers except the sender
    if (targetId === '*') {
  ...
  // Server-Side Relay
  // Fallback when WebRTC P2P cannot be established.
  ...
  #relayRequest(ws, clientId, roomId, data = {}) {
    ...
    // Enforce concurrent relay limit
    ...
    // Block duplicate relay from same sender
    ...
    const relay = {
      ...
      bytesTransferred: 0,
      startTime:        Date.now(),
      buffer:           [],          // chunks buffered before receiver connects
    };
  ...
  #relayData(ws, clientId, data = {}) {
    ...
    // Buffer until receiver is connected
    if (!relay.receiverWs) {
    ...
    try {
      ...
      this.#send(ws, {
        ...
        totalSize:        relay.metadata.fileSize,
      });
    } catch {
  ...
  #relayConnect(ws, clientId, data = {}) {
    ...
    // Flush buffered chunks to the now-connected receiver
  ...
  #handleDisconnection(clientId, roomId) {
    // End any relay this client was part of (as sender)
    this.#endRelay(clientId, 'disconnected');

    // End any relay this client was a target of (as receiver)
